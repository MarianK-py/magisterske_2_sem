...
-- pruneInv skontrolu ci sa v sudoku nenachaza pripad
-- ked na niektorej nie je single ale jedna z tych hodnot
-- sa nachadza iba na tej jednej pozicii v riadku
-- priklad:
-- [12, 3, 4, 25, 567, 567, 28, 9, 67]
-- v tomto pripade na prvej pozicii moze byt 1 alebo 2
-- ale kedze 1 nemoze byt nikde inde musi byt tam
-- podobne na siednmej pozicii je 2 8 ale 8 nikde inde nemoze byt
-- preto po tomto prunning bude vyzeraz riadok takto:
-- [1, 3, 4, 25, 567, 567, 8, 9, 67]
-- teraz sa situacia opakuje pre 2 na 4 pozicii cize po dalsom opakovani dostaneme
-- [1, 3, 4, 2, 567, 567, 8, 9, 67]
pruneInv  :: Matrix Choices -> Matrix Choices
pruneInv  =  pruneBy boxs . pruneBy cols . pruneBy rows
             where pruneBy f = f . map checkAllChoices . f

checkAllChoices :: Row Choices -> Row Choices
checkAllChoices xs = [checkChoices xs x | x <- xs]

checkChoices :: Row Choices -> Choices -> Choices
checkChoices xs x | single x = x
                  | otherwise = if (length ch) > 0 then [ch!!0] else x
                    where
                      ch = [y | y <- x, checkChoice xs y]

checkChoice :: Row Choices -> Value -> Bool
checkChoice xs x = (length $ filter (\y -> x `isin` y) xs) == 1

isin :: Value -> Choices -> Bool
x `isin` ys = any (\y -> x == y) ys

-- solver 5 sprvíme hybrid 3 a 4 pricom budeme alterovat
-- medzi prune a pruneInv kym nedojdeme do fixneho bodu
-- a potom pouzijeme seach (ktory tiez alterujeme medzi prunningmi)

combinedPruning :: Matrix Choices -> Matrix Choices
combinedPruning = pruneInv . prune

solver5  :: Grid -> [Grid]
solver5  =  search2 . fix combinedPruning . choices

search2  :: Matrix Choices -> [Grid]
search2 m
 | blocked m =  []
 | complete m =  collapse m
 | otherwise =  [g | m' <- expand_ m
               , g  <- search2 (combinedPruning m')]
