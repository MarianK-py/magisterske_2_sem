...
mostFrequent n s = let
                     sLow = map toLower s
                     words = splitWords (<'A') sLow
                     najcI :: Int -> [String] -> [String]
                     najcI n w | n == 0 = []
                               | otherwise = let
                                               naj = najcastejsi w
                                             in
                                               naj:(najcI (n-1) (filter (/= naj) w))
                   in
                     --(najcI n words)
                     init $ foldl (\acc x -> x++" "++acc) "" (najcI n words)



-- zobrate z mojich rieseni predchadzajucej DU
